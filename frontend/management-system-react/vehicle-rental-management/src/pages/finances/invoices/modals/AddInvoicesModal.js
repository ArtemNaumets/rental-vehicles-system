...
import * as InvoiceService from '../../../../api/FinanceService'; 
import '../../../../assets/css/style.css';
import '../../../../assets/css/AddModal.css';

const AddInvoicesModal = ({ show, onClose, onAdd, fields }) => {
    ...
    const addNewData = async () => {
        ...
        setLoading(true);
        try {
            await InvoiceService.saveInvoice(newData); 
            onClose();
            setNewData({});
            onAdd();
            setErrorMessage('');
    ...
    const renderSelect = (key, options) => {
        return (
            ...
            >
                <option value="" disabled>Select {key === 'clientid' ? 'client' : ''}</option>
                {options.map(option => (
                    <option key={option.id} value={option.id}>
                        {key.toLowerCase().includes('client') ? option.name : ''}
                    </option>
                ))}
            </select>
        );
