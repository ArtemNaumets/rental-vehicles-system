...
const AddStatesModal = ({ show, onClose, onAdd, fields }) => {
    ...
    const addNewData = async () => {
        ...
        setLoading(true);
        try {
            await LocationService.saveState(newData); 
            onClose();
            setNewData({});
            onAdd();
            setErrorMessage('');
    ...
    const renderSelect = (key, options) => {
        return (
            ...
            >
                <option value="" disabled>Select {key === 'countryid' ? 'country' : '' }</option>
                {options.map(option => (
                    <option key={option.id} value={option.id}>
                        {key.toLowerCase().includes('country') ? option.description : ''}
                    </option>
                ))}
            </select>
        );
